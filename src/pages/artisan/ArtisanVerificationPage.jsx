...
function ArtisanVerificationPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Add required files
      if (data.frontId && data.frontId[0]) formData.append("frontId", data.frontId[0]);
      if (data.backId && data.backId[0]) formData.append("backId", data.backId[0]);
      if (data.addressProof && data.addressProof[0]) formData.append("addressProof", data.addressProof[0]);
      if (data.businessReg && data.businessReg[0]) formData.append("businessReg", data.businessReg[0]);
        ...
        <div className="p-6 md:p-8">
          <div className="mb-8">
            ...
            <p className="text-gray-600 mb-4">
              To verify your identity as an artisan, please provide the following documents:
            </p>
              ...
              {verificationRequirements.map((req) => (
                ...
                >
                  <div className="flex items-start">
                    <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${req.required ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-500'}`}>
                      {req.required ? (
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                    {frontIdPreview ? (
                      ...
                      <div className="py-8">
                        ...
                        <p className="text-sm text-gray-500 mt-2">Upload front of your ID</p>
                      </div>
                    )}
                    <label className="inline-block px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 cursor-pointer">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        {...register("frontId", { required: "Front of ID is required" })}
                        onChange={(e) => handleFileChange(e, setFrontIdPreview)}
                      />
                    </label>
                    {errors.frontId && (
                      <p className="text-red-500 text-sm mt-2">{errors.frontId.message}</p>
                    )}
                  </div>
                </div>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                    {backIdPreview ? (
                      ...
                      <div className="py-8">
                        ...
                        <p className="text-sm text-gray-500 mt-2">Upload back of your ID</p>
                      </div>
                    )}
                    <label className="inline-block px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 cursor-pointer">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        {...register("backId", { required: "Back of ID is required" })}
                        onChange={(e) => handleFileChange(e, setBackIdPreview)}
                      />
                    </label>
                    {errors.backId && (
                      <p className="text-red-500 text-sm mt-2">{errors.backId.message}</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                  {addressProofPreview ? (
                    ...
                    <div className="py-8">
                      ...
                      <p className="text-sm text-gray-500 mt-2">Upload proof of address document</p>
                    </div>
                  )}
                  <label className="inline-block px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 cursor-pointer">
                    <span>{addressProofPreview ? "Change File" : "Select File"}</span>
                    <input
                      type="file"
                      accept="image/*"
                      className="hidden"
                      {...register("addressProof", { required: "Proof of address is required" })}
                      onChange={(e) => handleFileChange(e, setAddressProofPreview)}
                    />
                  </label>
                  {errors.addressProof && (
                    <p className="text-red-500 text-sm mt-2">{errors.addressProof.message}</p>
                  )}
                </div>
              </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                  {businessRegPreview ? (
                    ...
                    <div className="py-8">
                      ...
                      <p className="text-sm text-gray-500 mt-2">Upload business registration document</p>
                    </div>
                  )}
                  <label className="inline-block px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 cursor-pointer">
                    <span>{businessRegPreview ? "Change File" : "Select File"}</span>
                    <input
                      type="file"
                      accept="image/*"
                      className="hidden"
                      {...register("businessReg", { required: "Business registration document is required" })}
                      onChange={(e) => handleFileChange(e, setBusinessRegPreview)}
                    />
                  </label>
                  {errors.businessReg && (
                    <p className="text-red-500 text-sm mt-2">{errors.businessReg.message}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
                  <div className="text-center mb-4">
                    ...
                    <p className="text-sm text-gray-500 mt-2">Upload any additional supporting documents</p>
                  </div>
